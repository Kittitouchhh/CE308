...
// ─── Build native filter array for React Native 0.76+ ─────────────────────
// Returns an object with `experimental_filter` containing an array of native
// filter functions that get rendered at the pixel level (capturable by view-shot).
export function getFilterStyle(
  ...
  // Normalize to multiplier values (1 = no change)
  const b = 1 + brightness / 100;
  const c = 1 + contrast / 100;
  const s = 1 + saturation / 100;

  // Start with user slider adjustments
  ...
  switch (filter) {
    ...
    case 'sepia':
      ...
      break;
    case 'warm':
      filters.push({ sepia: 0.3 }, { saturate: 1.4 });
      break;
    case 'cool':
      filters.push({ hueRotate: '180deg' }, { saturate: 0.8 });
      break;
    case 'vivid':
      filters.push({ saturate: 2 }, { contrast: 1.1 });
      break;
    case 'vintage':
      filters.push({ sepia: 0.4 }, { contrast: 0.9 }, { brightness: 0.9 });
      break;
  }

  if (filters.length === 0) return {};
  return { experimental_filter: filters };
}
